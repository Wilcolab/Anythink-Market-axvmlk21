 ...
 * This function normalizes the input string by replacing underscores, hyphens, and multiple spaces
 * with a single space, then splits the string into words. All words are converted to lowercase and
 * joined together using hyphens as separators to form a kebab-case string.
 *
 * Edge cases handled:
 * - Throws an error if input is undefined, null, or not a string.
 * - Throws an error if the input string is empty after trimming.
 ...
 * @example
 * toKebabCase("hola que haces?"); // returns "hola-que-haces"
 * toKebabCase("hola   que   haces  ?"); // returns "hola-que-haces"
 * toKebabCase("hola---que--haces"); // returns "hola-que-haces"
 */
...
function toKebabCase(str) {
    ...
    // Filter out any empty words after cleaning
    const finalWords = cleanedWords.filter(word => word.length > 0);

    if (finalWords.length === 0) {
        throw new Error("No valid words found to convert to kebab-case.");
    }

    // Convert to kebab-case (all lowercase, joined by hyphens)
    return finalWords.map(word => word.toLowerCase()).join("-");
}

// Example usage:
try {
    console.log(toKebabCase("hola que haces?")); // hola-que-haces
    console.log(toKebabCase("hola   que   haces  ?")); // hola-que-haces
    console.log(toKebabCase("hola---que--haces")); // hola-que-haces
    console.log(toKebabCase("  multiple   spaces  here ")); // multiple-spaces-here
    console.log(toKebabCase(undefined)); // Throws error
} catch (err) {
    console.error("Error:", err.message);
}